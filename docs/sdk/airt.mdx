...
* **`variant_path`**
  ...
  –Path to Crescendo variant YAML (default: variant\_1).
* **`early_stopping_score`**
  (`float`, default:
  `0.8`
  )
  –Score threshold to stop early (0.0-1.0).
* **`context_depth`**
  (`int`, default:
  `5`
  )
  –Previous trials for context (for refinement).
* **`max_backtracks`**
  (`int`, default:
  `10`
  )
  –Max backtracks after refusals.
* **`initial_prompt`**
  (`str | None`, default:
  `None`
  )
...
<Accordion title="Source code in dreadnode/airt/attack/crescendo.py" icon="code">
```python
