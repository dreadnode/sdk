...
```python
ScorersLike = (
    Sequence[ScorerLike[T]] | Mapping[str, ScorerLike[T]]
)
```
...
```
@dn.task(scorers=[
dn.scorers.task\_input("text", lambda text: len(text) / 100) # Score based on length of input text
])
async def summarize(text: str) -> str:
...

Args:
input\_name: The name of the task input to use as the score.
...
<Accordion title="Source code in dreadnode/scorers/image.py" icon="code">
```python
def image_distance(
    reference: Image,
    method: DistanceMethod = "l2",
...
```python
json_path(
    expression: str, *, default: float | None = None
) -> Scorer[t.Any]
```
...
<Accordion title="Source code in dreadnode/scorers/json.py" icon="code">
```python
