...
```python
ToObject = Literal['task-or-run', 'run']
```
...
```python
console: ConsoleOptions | Literal[False, True] = console
```
...
```python
local_dir: str | Path | Literal[False] = local_dir
```

### otel\_scope

```python
otel_scope: str = otel_scope
```
...
```python
service_version: str | None = service_version
```
...
```python
__call__(func: Callable[P, Awaitable[R]]) -> Task[P, R]
```

```python
__call__(func: Callable[P, R]) -> Task[P, R]
```
