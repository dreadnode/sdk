...
* **`version`**
  ...
  –A specific version string to set for the dataset.
* **`strategy`**
  (`VersionStrategy | None`, default:
  `None`
  )
  –A versioning strategy to automatically bump the version
...
* **`overwrite`**
  ...
  –Whether to overwrite an existing dataset version at the same path.
