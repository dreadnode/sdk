...
## Using Scorers with Rigging

Strikes integrates seamlessly with [Rigging](/open-source/rigging/intro), allowing you to score LLM outputs at every stage of your chat pipeline. This integration supports both traditional task-based scoring and inline pipeline scoring.

### Scoring Chat Objects with `wrap_chat`
...
### Pipeline-Level Scoring with `.score()`

The most powerful way to use scorers with Rigging is through the ChatPipeline's `.score()` method. This allows you to attach scorers directly to your pipeline, which will automatically evaluate the generated chat upon completion.
...
pipeline = (
    ...
    .chat("Summarize this text in 2-3 sentences: " + text)
    .score(
        dn.scorers.llm_judge(
            "gpt-4o",
            "Rate the summary between 0 and 5 based on clarity and completeness.",
            passing=lambda score: score >= 3
        ),
        dn.scorers.contains("key_concept"),  # Ensure key concepts are preserved
        dn.scorers.length_target(100),       # Target around 100 characters
        filter="last_assistant"              # Only score the final response
    )
)

# Run the pipeline with automatic scoring
chat = await pipeline.run()
```

### Message Filtering Options

The `filter` parameter in `.score()` controls which messages are evaluated:

- **`"all"`**: Use all messages in the chat
- **`"last"`**: Use only the last message (default)
...
- **Custom function**: A callable that takes `list[Message]` and returns filtered `list[Message]`

```python
# Custom filter example: score only messages longer than 50 characters
def long_messages_only(messages):
    return [msg for msg in messages if len(str(msg.content)) > 50]

pipeline = (
    rg.get_generator("gpt-4o")
    .chat("Write a detailed explanation...")
    .score(
        dn.scorers.readability(target_grade=8),
        filter=long_messages_only
    )
)
```

### Using Lookups with Rigging

Lookups work seamlessly with Rigging pipelines, allowing dynamic scoring based on pipeline context:

```python
# Score based on the original user input
pipeline = (
    rg.get_generator("gpt-4o")
    .chat("Rephrase this more professionally: " + user_input)
    .score(
        # Compare similarity to original input
        dn.scorers.similarity(
            reference=dn.lookup_input("messages",
                                    process=lambda m: m[-1].content)
        ),
        # Ensure the rephrasing maintains key concepts
        dn.scorers.llm_judge(
            "gpt-4o-mini",
            "Does this maintain the core meaning of the original?",
...
Available inputs on pipelines include:

- **`"messages"`**: The full list of messages in the chat
- **`"generator_id"`**: The model identifier used
