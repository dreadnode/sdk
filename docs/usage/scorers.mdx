...
Here is a complete example of a configurable custom scorer that handles `Lookup`s:

```python
import typing as t
from difflib import SequenceMatcher
from dreadnode import Scorer, Lookup, resolve, lookup_input
...
def custom_similarity(reference: str) -> Scorer[t.Any]:
    ...
    async def evaluate(data_to_score: t.Any) -> float:
        return SequenceMatcher(a=reference, b=text(data_to_score)).ratio()
...
The solution is the `dreadnode.resolve()` utility function. It safely resolves a value that might be a `Lookup`:

- If the value is a `Lookup`, `resolve()` fetches the actual value from the run/task context.
- If the value is already a direct value (like a string or number), `resolve()` simply returns it unchanged.

By wrapping your scorer's parameters with `resolve()`, you can transparently handle both static values and dynamic `Lookup`s.
