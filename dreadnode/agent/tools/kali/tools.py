...
@tool
def enumerate_users_netexec(target: str, username: str, password: str, domain: str) -> str:
    """
    ...
    try:
        ...
        raise TimeoutError(f"User enumeration timed out for {target}")
    except Exception as e:
        logger.error(
...
def enumerate_shares_netexec(
    ...
    try:
        ...
        raise TimeoutError(f"Share enumeration timed out for {target}")
    except Exception as e:
        logger.error(
...
def secretsdump(
    ...
    try:
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=timeout_minutes * 60, env=env, check=False
        )
    ...
    logger.info(f"[*] Secretsdump completed for {target} with {username} result: {result.stdout}")
    return result.stdout
...
def asrep_roast(
    ...
    temp_userfile = None
    if user_list:
        temp_userfile = f"/tmp/asrep_users_{domain.replace('.', '_')}.txt"
    ...
    try:
        ...
        if temp_userfile and os.path.exists(temp_userfile):
            os.remove(temp_userfile)
...
def john_the_ripper(
    ...
    Args:
        hash_value: Hash to crack
        hash_format: John hash format to use (e.g., 'krb5asrep', 'krb5tgs', 'ntlm', 'md5')
        wordlist_path: Path to wordlist file (default: /usr/share/wordlists/rockyou.txt)
        max_time_minutes: Maximum time to spend cracking

    Returns:
        String output from john

    Example:
        >>> result = john_crack("$krb5asrep$23$john.oliver@CC...",
        ...                     "krb5asrep", "/usr/share/wordlists/rockyou.txt")
    """
    ...
    try:
        ...
        try:
            ...
            if show_result.stdout.strip():
                output = "\nCracked passwords:\n" + show_result.stdout
            logger.info(f"[*] John the Ripper completed for {hash_value} result: {output}")
            return output
...
def domain_admin_checker(
    ...
    try:
        ...
        if password:
            print(f"[*] Domain admin checker using password for {username}")
            cmd.extend(["-u", username, "-p", password])
        elif hash:
            print(f"[*] Domain admin checker using hash for {username}")
        ...
        raise TimeoutError(f"Domain admin checker timed out for {targets}")
    except Exception as e:
        logger.error(
