            ...
            while True:
                ...
                if not chunk:
                    ...
                    if buffer:  # Handle any remaining data in buffer
                        ...
                        await self.output_queue.put(
                            (current_time, decoded_buffer, is_stderr, self.track_time)
                        )
                ...
                for i, line in enumerate(lines):
                    ...
                    if (
                        decoded_line == "\n"
                        and i > 0
                        and lines[i - 1].decode("utf-8", errors="replace") == "\n"
                    ):
                        continue
                    ...
                    await self.output_queue.put(
                        (current_time, decoded_line, is_stderr, self.track_time)
                    )
        ...
        try:
            ...
            try:
                ...
                await asyncio.wait_for(
                    asyncio.gather(self.process.wait(), stdout_reader(), stderr_reader()),
                ...
                if self.timeout < self.max_timeout:
                    ...
                    self.timeout_reason = (
                        f"Command exceeded maximum execution time of {self.timeout}s"
                    )
...
async def run_async_command(
    cmd, timeout, logger, display_command=None, max_timeout=None, track_time=False
):
    ...
    async def _run():
        ...
        if current_line:
            start_ts = current_line_timestamp or (
                sorted_items[-1][0] if sorted_items else manager.start_time
            )
            elapsed = start_ts - manager.start_time
...
class DockerExecutor(BaseExecutor):
    ...
    def __init__(
        ...
        super().__init__(
            default_timeout=default_timeout, max_timeout=max_timeout, use_tmux=use_tmux
        )
        ...
        self.container_name = (
            f"{self.base_container_name}-{self.architecture}"
        ...
        self.capabilities: list[str] = [
            "NET_ADMIN"
        ]  # Default capability needed for network scanning and VPN operations
    ...
    def _verify_container_architecture(self) -> bool:
        ...
        try:
            ...
            expected_architecture = (
                f"-{self.architecture}" if self.architecture != "default" else ""
            )
            if expected_architecture and expected_architecture not in container_image:
                self.logger.warning(
                    f"Container {self.container_name} is running with image {container_image}, "
    ...
    def _dockerfile_changed(self) -> bool:
        ...
        if current_hash != stored_hash:
            self.logger.info(
                f"Dockerfile {self.dockerfile_path} hash changed from {stored_hash} to {current_hash}"
            )
        ...
        try:
            ...
            if self.is_running():
                ...
                if not self._verify_container_architecture():
                    self.logger.info(
                        f"Container running with incorrect architecture. Stopping it to recreate with {self.architecture} architecture."
                    )
                    self.stop()
                else:
                    self.logger.info(
                        f"Reusing existing container with {self.architecture} architecture."
                    )
                    return True
            ...
            if hasattr(self, "custom_image") and self.custom_image:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to pull image {self.custom_image}: {e}. Will try to use local image if available."
                    )
            # Build/rebuild image if needed
            elif not self._image_exists() or self._dockerfile_changed():
...
"""
    ...
    def write_file(self, dest_path: str, content: bytes) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Unexpected error writing file to container: {e!s}")
            return False
        finally:
            ...
            if "temp_file_path" in locals():
                try:
                    ...
                    self.logger.warning(f"Error cleaning up temporary file: {e!s}")
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "image",
                    "inspect",
                    f"{self.image_tag}:latest",
                ],  # Use image_tag instead of container_name
                capture_output=True,
                check=True,
            )
            return True
