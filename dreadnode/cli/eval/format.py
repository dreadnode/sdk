...
def format_evals_table(evals: list[Eval]) -> RenderableType:
    """
    ...
    for evaluation in evals:
        tool_names = ", ".join(tool.name for tool in evaluation.tools) if evaluation.tools else "-"
        table.add_row(
            evaluation.name,
            evaluation.description or "-",
            # Show only the primary model for brevity in the table
            (eval.model or "-").split(",")[0],
            tool_names,
        )
...
def format_eval(evaluation: Eval) -> RenderableType:
    ...
    details.add_row(Text("Model", justify="right"), evaluation.model or "-")
    details.add_row(
        Text("Instructions", justify="right"),
        f'"{shorten_string(evaluation.instructions, 100)}"' if evaluation.instructions else "-",
    )
