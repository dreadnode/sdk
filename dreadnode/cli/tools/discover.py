...
def discover_tools(path: pathlib.Path = DEFAULT_TOOL_SEARCH_PATH) -> list[str]:
    """Discover available tools in the specified directory.

    Args:
        path (pathlib.Path): The directory to search for tools.

    Returns:
        t.List[str]: A list of discovered tool names.
    """
    ...
    tool_files = list(path.glob("runner/playbooks/*-tool.yml"))

    return [file.stem.replace("-tool", "") for file in tool_files]
