    ...
    **To get detailed help for a specific task, use `dreadnode task run <task> help`.**
    ...
    if config:
        ...
        if config.suffix in {".toml"}:
            task_app._config = cyclopts.config.Toml(config, use_commands_as_keys=False)  # noqa: SLF001
        elif config.suffix in {".yaml", ".yml"}:
            task_app._config = cyclopts.config.Yaml(config, use_commands_as_keys=False)  # noqa: SLF001
        elif config.suffix in {".json"}:
            task_app._config = cyclopts.config.Json(config, use_commands_as_keys=False)  # noqa: SLF001
        else:
