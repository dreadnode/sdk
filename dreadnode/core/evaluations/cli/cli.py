...
evaluation_cli = cyclopts.App("eval", help="Discover and run evaluations.")


@evaluation_cli.command(name=["list", "ls", "show"])
def show(
    file: Path | None = None,
    *,
    verbose: t.Annotated[
    ...
    discovered = discover(Evaluation, file)
    if not discovered:
...
async def run(  # noqa: PLR0912, PLR0915
    ...
    **To get detailed help for a specific eval, use `dreadnode eval run <eval> help`.**
    ...
    """
    from dreadnode.evaluations import Eval
    ...
    evaluation_cli.__annotations__["config"] = config_annotation
    ...
    eval_app.default(evaluation_cli)
