    ...
    *,
    ...
    try:
        ...
        if not capture_output:
            logger.info("")  # Some padding after command output
...
def get_local_registry_port() -> int:
    for container in docker_ps():
        if settings.docker_registry_image_tag in container.image and container.ports:
            # return the first mapped port
            return container.ports[0][0]

    # fallback to the default port
    return settings.docker_registry_local_port
...
def get_registry(config: ServerConfig) -> str:
    ...
    return f"{prefix}{settings.docker_registry_subdomain}.{settings.platform_base_domain}"
