...
from dreadnode.core.evaluations.result import EvalResult
...
class Trial(BaseModel, t.Generic[CandidateT]):
    ...
    Attributes:
        ...
        scores: A dictionary of all named objective scores for this trial.
        directional_scores: A dictionary of all named objective scores adjusted
        ...
        parent_id: The id of the parent trial, as defined by the search strategy.
        is_probe: Whether this trial is a probe used for intermediate evaluation.
    ...
    def cost(self) -> int:
        ...
        if self.evaluation_result:
            return len(self.evaluation_result.samples)
        return 0
    ...
    def output(self) -> t.Any | None:
        ...
        if self.evaluation_result and self.evaluation_result.samples:
            return self.evaluation_result.samples[0].output
        return None
