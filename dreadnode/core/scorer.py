...
from dreadnode.core.exceptions import AssertionFailedError, warn_at_user_stacklevel
from dreadnode.core.meta import Component, ConfigInfo, Context
from dreadnode.core.meta.context import TaskInput, TaskOutput
from dreadnode.core.metric import (
    Metric,
    T,
)
from dreadnode.core.types.common import UNSET, AnyDict, JsonDict, Unset
from dreadnode.core.util import clean_str, get_callable_name
...
class Scorer(Component[te.Concatenate[T, ...], t.Any], t.Generic[T]):
    ...
    def with_(
        ...
        *,
        ...
        new.assert_ = assert_ if assert_ is not None else self.assert_
    ...
    def assert_on(self) -> "Scorer[T]":
        """Mark this scorer as an assertion (must be truthy)."""
        return self.with_(assert_=True)

    def assert_off(self) -> "Scorer[T]":
        """Mark this scorer as not an assertion."""
        return self.with_(assert_=False)
    ...
    def as_scorer(
        self: "Scorer[T]",
        func: t.Callable[[OuterT], T],
        obj: type[OuterT] = t.Any,  # type: ignore[assignment] # noqa: ARG002
        *,
        name: str | None = None,
    ) -> "Scorer[OuterT]":
        """
        ...
        async def evaluate(obj: OuterT, *args: t.Any, **kwargs: t.Any) -> list[Metric]:
            return await original.normalize_and_score(func(obj), *args, **kwargs)
    ...
    async def evaluate(
        ...
        *,
        step: int | None = None,
        assert_scores: t.Literal[True, False] | list[str] | None = None,
        ...
        Args:
            obj: The object to score.
            scorers: A list of scorers to use.
            step: An optional step value to attach to all generated metrics.
            assert_scores: Controls assertion behavior:
        ...
        _scorers = cls.fit_many(scorers)

        # Determine which scorers to assert
        if assert_scores is False:
            # Disable all assertions
            _assert_names: set[str] = set()
        elif assert_scores is True:
            # Assert all scorers
            _assert_names = {s.name for s in _scorers}
        elif assert_scores is not None:
            # Explicit list overrides scorer.assert_
            _assert_names = set(assert_scores)
        else:
            # Default: use each scorer's assert_ field
            _assert_names = {s.name for s in _scorers if s.assert_}

        task = current_task_span.get()

        metrics: dict[str, list[Metric]] = {}
        nested_metrics = await asyncio.gather(
            *[scorer.normalize_and_score(obj) for scorer in _scorers]
        )
        ...
        for scorer, _metrics in zip(_scorers, nested_metrics, strict=True):
            for metric in _metrics:
                ...
                if task is not None:
                    task.log_metric(metric_name, metric, origin=scorer.bound_obj or obj)

                metrics.setdefault(metric_name, []).append(metric)

        # Check assertions
        failed_assertions: dict[str, list[Metric]] = {}
        for name in _assert_names:
            metric_list = metrics.get(name, [])
            if not metric_list:
                # Try to find by scorer name in case of nested metrics
...
def scorer(
    ...
    *,
    ...
    def make_scorer(func: ScorerCallable[T]) -> Scorer[T]:
        ...
        return Scorer(func, name=name, assert_=assert_, attributes=attributes)
