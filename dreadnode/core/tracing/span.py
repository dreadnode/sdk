...
from dreadnode.core.settings import DEFAULT_MAX_INLINE_OBJECT_BYTES
...
from dreadnode.core.tracing.convert import run_span_to_graph
...
class Span(ReadableSpan):
    ...
    def __exit__(
        ...
        try:
            ...
            # Token created in different async context - span data already recorded
            pass
        self._token = None
...
class RunSpan(Span):
    ...
    def _store_file_by_hash(self, data_bytes: bytes, full_path: str) -> str:
        ...
        def store_operation() -> str:
            filesystem = self._storage._get_filesystem()  # type: ignore[union-attr]
        ...
        return self._storage.execute_with_retry(store_operation)
    ...
    def _create_object_by_hash(self, serialized: Serialized, object_hash: str) -> Object:
        ...
        prefix = self._storage.get_prefix()
    ...
    def log_artifact(
        ...
        *,
        ...
        """
        Log a file as an artifact, storing in workspace CAS and uploading to remote.

        Args:
            local_uri: Path to the local file to log.
            name: Optional name for the artifact (defaults to filename).

        Returns:
            Artifact metadata dict, or None if the file doesn't exist.
        ...
        if local_path.is_dir():
            # For directories, log each file recursively
            artifacts = []
...
class TaskSpan(Span, t.Generic[R]):
    ...
    def log_metric(
        ...
        *,
        ...
        #
        # Let the run handle the origin and aggregation
        ...
        if (run := current_run_span.get()) is not None:
            metric = run.log_metric(
                key, metric, prefix=self._label, origin=origin, aggregation=aggregation
            )

        series = self._metrics.setdefault(key, MetricSeries())
        series.append(value=metric.value, step=metric.step)

        return metric

    def get_average_metric_value(self, key: str) -> float:
        """Get the mean of the specified metric series."""
